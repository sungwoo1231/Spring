...
public class EmployeeTemplateRepository implements EmployeeRepository {
    ...
    private final RowMapper<Employee> employeeRowMapper = new RowMapper<Employee>() {
        ...
        public Employee mapRow(ResultSet rs, int rowNum) throws SQLException {
            ...
            employee.setHomePhoneNumber(rs.getString("집전화"));
            employee.setSupervisorDepartmentNumber(rs.getString("상사번호"));
...
//        return jdbcTemplate.query(query,mapper);
    ...
    public Employee saveEmployee(Employee employee) {
        String query = "insert into 사원(사원번호,이름,영문이름,직위,성별,생일,입사일,주소,도시,지역,집전화,상사번호,부서번호) "
                + "values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?) " ;
        jdbcTemplate.update(query,employee.getEmployeeNumber(),
                employee.getName(),
                ...
                employee.getCity(),
                employee.getArea(),
                employee.getHomePhoneNumber(),
                employee.getSupervisorDepartmentNumber(),
